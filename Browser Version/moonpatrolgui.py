...
class MoonPatrolGui:
    def __init__(self, g: MoonPatrolGame):
        ...
        (self._key_1, self._key_2, self._key_start_game, self._key_go_back, self._key_up_1, self._key_left_1, self._key_stay_1, self._key_right_1, self._key_bullet_1,
         self._key_up_2, self._key_left_2, self._key_stay_2, self._key_right_2, self._key_bullet_2, self._key_restart) = self._game.commands()
            ...
            if g2d.key_pressed(self._key_restart) and self._game.finished():
                ...
                for a in self._arena.actors():
                    if self._game.actor_type(a) == constants.ROVER:
                        ...
                        if g2d.key_pressed(up.lower()) or g2d.key_pressed(up.upper()):
                            a.jump()
                        elif g2d.key_pressed(right.lower()) or g2d.key_pressed(right.upper()):
                            a.go_right()
                        elif g2d.key_pressed(left.lower()) or g2d.key_pressed(left.upper()):
                            a.go_left()
                        elif (g2d.key_released(right.lower()) or g2d.key_released(right.upper()) or g2d.key_released(left.lower()) or
                            g2d.key_released(left.upper()) or g2d.key_pressed(stay.lower()) or g2d.key_pressed(stay.upper())):
                            a.stay()
                            
                        if g2d.key_pressed(invincible.lower()) or g2d.key_pressed(invincible.upper()):
                            a.set_invincible() #Just for testing purposes, I swear I never cheated.. almost never

                        if (g2d.key_pressed(bullet.lower()) or g2d.key_pressed(bullet.upper())) and self._game.count_bullets() < constants.MAX_BULLETS and not a.damaged():
    ...
    def score(self):
        ...
        g2d.draw_text(str(self._score), (75, 10), constants.MEDIUM_FONT_SIZE)
        g2d.draw_image_clip(self._sprite, (161, 36, 89, 14), (self._arena_width - 100, 10, 60, 9)) #Level
        g2d.draw_text(str(self._level), (self._arena_width - 35, 10), constants.MEDIUM_FONT_SIZE)
